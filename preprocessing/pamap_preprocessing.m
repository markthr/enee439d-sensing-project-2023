...
output_path = path + "/preprocessed/pamap_";
...
% get letter alias for the 18 activities in the PAMAP2 dataset
activities = char([1:7, 9:13, 16:20, 24] + 'A' - 1);
n_act= length(activities);

time_scale = 1; % seconds to nanos
fs = 20;
%% load directories
sensor_paths = struct;
% use hand as stand-in for WISDM's watch
sensor_paths.w_acc = dir(path + "/mat/hand/accel/*.mat");
sensor_paths.w_gyr  = dir(path + "/mat/hand/gyro/*.mat");
% use chest as stand-in for WISDM's watch
sensor_paths.p_acc = dir(path + "/mat/chest/accel/*.mat");
sensor_paths.p_gyr = dir(path + "/mat/chest/gyro/*.mat");

sensor_names = ["Hand Acceleration", "Hand Gyro", "Chest Acceleration", "Chest Gyro"];
fn = fieldnames(sensor_paths);
n_subj = length(sensor_paths.(fn{1}));
% check data to ensure subjects line up
get_subj = "^subject(\d{3})\.mat$";
...
%%
seq = 1:500;
nustft(xyz_to_mat(ds.(fn{i_sens}), seq), single(ds.(fn{i_sens}).TimeStamp(seq)), fs, window_time, window_time_shift, max_window_error)
%%
...
for file_index = 1:n_subj
    ...
    % PAMAP2 has widely varying lengths, add them up
    data_length = 1;
    for activity_index = 1:n_act
        ...
        activity_time = 0;
        if(numel(ds) ~= 0)
            % get longest time of any sensors recordings
            for i = 1:numel(fn)
                % check that there is data, add to length if so
                if(~isempty(ds.(fn{i})))
                    candidate_time = ds.(fn{i}).TimeStamp(end) - ds.(fn{i}).TimeStamp(1);
                    if(candidate_time > activity_time)
                        activity_time = candidate_time;
                    end
                end
            end
        end

        data_length = data_length + ceil(activity_time);
    end
    output = table('Size',[data_length, n_cols],'VariableTypes', col_types, 'VariableNames', col_names);
    ...
    for activity_index = 1:n_act
        ...
        for i_sens = 1:numel(fn)
            timestamps = single(ds.(fn{i_sens}).TimeStamp);
            if (length(timestamps) < 100)
                % too little data for processing, break and skip rest of
                % sensors
                break
            end
            [s2, t] = nustft(xyz_to_mat(ds.(fn{i_sens})), timestamps, fs, window_time, window_time_shift, max_window_error);
            % save results to table
            t_len = numel(t);
            output.Time(row_offset + (1:t_len)) = t;
            output.SubjectID(row_offset + (1:t_len)) = str2num(subject_data.SubjectID);
            ...
            % flatten frequency matdrix at every time and calculate db
...
function activity_data_struct = load_activity(subject_data_struct, activity)
    ...
    for i = 1:numel(fn)
        ...
        if (isfield(sensor.activity_data, activity))
            activity_data_struct.(fn{i}) = sensor.activity_data.(activity);
        else
            activity_data_struct.(fn{i}) = {};
        end
    end
...
function new_timestamps = remove_offset(timestamps, scale)
    new_timestamps = timestamps - floor(timestamps(1)/scale);
end
...
function activity_data_struct = align_sensor_times(activity_data_struct, time_scale)
    ...
    beginnings = zeros(1,4, 'double');
    % remove the part of offset that is greater than 1 second
    for i = 1:n
        activity_data_struct.(fn{i}).TimeStamp = ...
            remove_offset(activity_data_struct.(fn{i}).TimeStamp, time_scale);
        beginnings(i) = activity_data_struct.(fn{i}).TimeStamp(1);
    end
    
    % calculate resulting delays for all choices of t_0
    delays = zeros(n,n, 'double');
    for i = 1:n
    ...
    alignments = beginnings(index) - zeros(1, n, 'double')  ;
    alignments(mask(index, :)) = alignments(mask(index, :)) - time_scale;

    % update time for each sensor
    for i = 1:n
        activity_data_struct.(fn{i}).TimeStamp = ...
            activity_data_struct.(fn{i}).TimeStamp - alignments(i);
    end
end
...
function [s2, t] = nustft(x, t, fs, window_time, window_time_shift, max_window_error)
    ...
    parfor i = window_indices
        ...
        bin_size = idivide(n, nufft_length);
        if(bin_size == 1)
            s2(i,:,:) = abs(Y .* conj(Y));
        else
